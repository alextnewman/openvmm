...
async fn rss_reconfig_preserves_rx_buffer_stream(driver: DefaultDriver) {
    ...
    let rx1 = loopback_recv(&mut queue, &mut pool, &payload_mem, std::slice::from_ref(&sent1)).await;
    assert_eq!(
    ...
    for (rx_id, expected) in rx23.iter().zip([&sent2, &sent3]) {
        ...
        payload_mem
            ...
            .unwrap();
        assert_eq!(&got, expected, "payload mismatch on {rx_id:?} after reconfig");
    }
...
impl Endpoint for SteeringEndpoint {
    ...
    async fn get_queues(
    ...
    ) -> anyhow::Result<()> {
        {
            ...
            switch.indir = rss.map(|r| r.indirection_table.to_vec()).unwrap_or_default();
            switch.queues.clear();
...
impl Queue for SteeringQueue {
    ...
    fn rx_avail(&mut self, _pool: &mut dyn BufferAccess, done: &[RxId]) {
        let mut switch = self.switch.lock();
        switch.queues[self.index].rx_avail.extend(done.iter().copied());
    }
