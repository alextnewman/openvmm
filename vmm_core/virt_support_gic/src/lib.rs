...
mod gicr {
    ...
    mod tests {
        ...
        fn reset_clears_cpu_interface() {
            ...
            redist.write_cpuif(SystemReg::ICC_AP0R0_EL1, u32::MAX.into());
            redist.write_cpuif(SystemReg::ICC_AP1R0_EL1, u32::MAX.into());
            redist.reset();
            assert_eq!(redist.read_cpuif(SystemReg::ICC_PMR_EL1), Some(0));
            assert_eq!(redist.read_cpuif(SystemReg::ICC_IGRPEN1_EL1), Some(0));
            assert_eq!(redist.read_cpuif(SystemReg::ICC_AP0R0_EL1), Some(0));
            assert_eq!(redist.read_cpuif(SystemReg::ICC_AP1R0_EL1), Some(0));
            assert_eq!(
                redist.read_cpuif(SystemReg::ICC_RPR_EL1),
                Some(super::IDLE_PRIORITY.into())
            );
        }
